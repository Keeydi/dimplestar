...
<?php
	...
	// Fetch distinct origins and destinations from DB
	$origins = [];
	$destinations = [];
	if(isset($conn) && $conn){
		$originResult = mysqli_query($conn, "SELECT DISTINCT origin FROM routes ORDER BY origin ASC");
		if($originResult){
			while($row = mysqli_fetch_assoc($originResult)){
				$origins[] = $row['origin'];
			}
		}
		$destinationResult = mysqli_query($conn, "SELECT DISTINCT destination FROM routes ORDER BY destination ASC");
		...
		<tr>
			...
			<td colspan="3">
				<select name="Origin">
					...
					<?php
						$selectedOrigin = isset($_POST['Origin']) ? $_POST['Origin'] : (isset($_SESSION['Origin']) ? $_SESSION['Origin'] : '0');
						foreach($origins as $o){
							$sel = ($o === $selectedOrigin) ? ' selected' : '';
		...
		<tr>
			...
			<td colspan="3">
				<select name="Destination">
					<option value="0">Select</option>
					<?php
						$selectedDestination = isset($_POST['Destination']) ? $_POST['Destination'] : (isset($_SESSION['Destination']) ? $_SESSION['Destination'] : '0');
						foreach($destinations as $d){
							$sel = ($d === $selectedDestination) ? ' selected' : '';
							echo "<option value=\"".htmlspecialchars($d, ENT_QUOTES)."\"$sel>".htmlspecialchars($d)."</option>";
						}
					?>
				</select>
			</td>
